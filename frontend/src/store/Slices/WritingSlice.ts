...
export const fetchPromptById = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>('writing/fetchPromptById', async (id, { rejectWithValue }) => {
  try {
...
export const fetchSubmission = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>('writing/fetchSubmission', async (id, { rejectWithValue }) => {
  try {
...
export const writingSlice = createSlice({
  ...
  reducers: {
    ...
    setFilter: (
      state,
      action: PayloadAction<{ key: 'level' | 'type'; value: string | null }>,
    ) => {
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(fetchSubmissions.pending,   (state) => { state.submissionsLoading = true; })
      .addCase(fetchSubmissions.fulfilled, (state, action) => {
        state.submissionsLoading = false;
        state.submissions        = action.payload;
      })
      .addCase(fetchSubmissions.rejected,  (state) => { state.submissionsLoading = false; });
  },
});
