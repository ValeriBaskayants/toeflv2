...
class ListeningSegmentDto {
  ...
  @IsNumber() @Min(0)    startSec!: number;
  @IsNumber() @Min(0)    endSec!:   number;
...
class CreateListeningQuestionDto {
  ...
  @IsOptional() @IsNumber() @Min(0)   referenceStartSec?: number;
  @IsOptional() @IsNumber() @Min(0)   referenceEndSec?:   number;
}
...
export class CreateListeningMaterialDto {
  ...
  @IsEnum(ListeningType)             type!:     ListeningType;
  @IsString() @IsNotEmpty()          fullText!: string;

  @IsArray() @ValidateNested({ each: true }) @Type(() => ListeningSegmentDto)
  segments!: ListeningSegmentDto[];

  @IsOptional() @IsNumber() @Min(0.1) @Max(2.0) speakerRate?:  number;
  @IsOptional() @IsString()                      speakerLang?:  string;
  @IsOptional() @IsNumber() @Min(0.5) @Max(2.0) speakerPitch?: number;

  @IsArray() @IsEnum(ListeningMode, { each: true })
  ...
  @IsOptional()
  @IsArray() @ValidateNested({ each: true }) @Type(() => CreateListeningQuestionDto)
  questions?: CreateListeningQuestionDto[];
}

export class BulkCreateListeningDto {
  @IsArray() @ValidateNested({ each: true }) @Type(() => CreateListeningMaterialDto)
