...
import { LEVEL_ORDER, LEVEL_DISPLAY, LEVEL_REQUIREMENTS } from '../../constants/level-requirements';
...
const BONUS_RESOURCES: Readonly<Record<Level, ExternalResource[]>> = {
  A1: [
    {
      title: 'BBC Learning English — Beginners',
      url: 'https://www.bbc.co.uk/learningenglish',
      type: 'website',
      description: 'Extra reading and listening for A1',
    },
  ],
  A1_PLUS: [
    {
      ...
      type: 'podcast',
      description: 'Short real English conversations',
    },
  ],
  A2: [
    {
      title: 'British Council LearnEnglish A2',
      url: 'https://learnenglish.britishcouncil.org/grammar/a1-a2-grammar',
      type: 'website',
      description: 'Extra grammar practice',
    },
  ],
  A2_PLUS: [
    {
      ...
      type: 'podcast',
      description: 'Slow-speed real news for listening practice',
    },
  ],
  B1: [
    {
      title: 'TED-Ed Short Talks',
      url: 'https://ed.ted.com',
      type: 'video',
      description: '5–10 min educational videos with subtitles',
    },
  ],
  B1_PLUS: [
    {
      title: 'British Council B1 Grammar',
      url: 'https://learnenglish.britishcouncil.org/grammar/b1-b2-grammar',
      type: 'website',
      description: 'Extra B1 grammar drills',
    },
  ],
  B2: [
    {
      ...
      type: 'video',
      description: 'Full-speed native speaker talks',
    },
  ],
  B2_PLUS: [
    {
      title: 'BBC Sounds Podcasts',
      url: 'https://www.bbc.co.uk/sounds',
      type: 'podcast',
      description: 'Native BBC radio for advanced listening',
    },
  ],
  C1: [
    {
      ...
      type: 'website',
      description: 'Authentic academic English reading',
    },
  ],
  C2: [
    {
      title: 'ETS TOEFL Official Practice',
      url: 'https://www.ets.org/toefl',
      type: 'website',
      description: 'Official TOEFL practice tests',
    },
  ],
};
...
function computeSMS(
  ...
  const qualityMultiplier =
    accuracyMin <= 0 ? 1.0 : Math.max(0.3, Math.min(1.0, accuracy / accuracyMin));
  ...
  async getRoadmap(userId: string): Promise<RoadmapResponse> {
    ...
    const nodes: RoadmapLevelNode[] = LEVEL_ORDER.map((level, idx) => {
      const status: RoadmapLevelNode['status'] =
        idx < currentLevelIdx ? 'completed' : idx === currentLevelIdx ? 'current' : 'locked';
        ...
        status === 'current'
          ? [
              ...
              },
              {
                ...
                isBlocking:
                  req.vocabulary.required > 0 && v.learned < req.vocabulary.required * 0.5,
              },
              {
                skill: 'reading',
              ...
              },
              {
                ...
                isBlocking:
                  computeSMS(q.completed, req.quiz.required, q.accuracy, req.quiz.accuracyMin) < 50,
              },
            ]
          : buildSkeletonSkills(level, status);

      const readinessPercent =
        status === 'completed' ? 100 : status === 'locked' ? 0 : computeReadinessFromSkills(skills);
    ...
    const projectedDate = this.computeProjectedDate(nodes, currentLevelIdx, avgMinutesPerDay);
