...
export class GrammarRulesService {
  ...
  async findAll(params: {
    ...
    const exerciseCountByTopic = new Map(exerciseCounts.map((g) => [g.topic, g._count._all]));
    const mistakeByTopic = new Map(userMistakes.map((m) => [m.topic, m]));
  ...
  async findBySlug(slug: string, userId: string): Promise<GrammarRuleDetail> {
    ...
    if (userMistake !== null) {
      ...
      userAccuracy = total > 0 ? Math.round((userMistake.correctCount / total) * 100) : null;
    }
  ...
  async getTopicsForLevel(level: Level): Promise<
    Array<{
      slug: string;
      topic: string;
      summary: string;
