...
>('admin/importContent', async ({ type, data }, { rejectWithValue }) => {
  try {
    const call = (() => {
      switch (type) {
        case 'exercises':
          return exercisesApi.bulkCreate(data);
        case 'grammarRules':
        ...
        case 'multipleChoice':
          return MultipleChoiceApi.bulkCreate(data);
        case 'writingPrompts':
...
export const selectImportState = (type: ContentType) => (s: AdminRootState) =>
  s.admin.imports[type];
