...
import {
  computeXP, XP_BASE, isSessionCountable, DIFFICULTY_XP_MULTIPLIER,
...
export class MultipleChoiceService {
  ...
  async submitSession(
    ...
    const topicHits = new Map<string, { correct: number; total: number }>();
    ...
    const results = dto.answers
      .map((a) => {
        ...
        for (const slug of question.topicSlugs) {
          const hit = topicHits.get(slug) ?? { correct: 0, total: 0 };
          hit.total += 1;
          if (isCorrect) hit.correct += 1;
          topicHits.set(slug, hit);
        }
    ...
    if (countedAsCompleted) {
      await this.progress.recordSkillCompletion({
        userId, skill: 'quiz', accuracy, xpEarned, timezone,
      });
    } else {
    ...
    if (topicHits.size > 0) {
      await Promise.all(
        Array.from(topicHits.entries()).map(([slug, { correct, total }]) =>
          this.topicMastery.recordPractice({
            userId,
            topicSlugs: [slug],
            kind:       'quiz',
    ...
    return {
      ...
      xpEarned,
      countedAsCompleted,
      feedback: buildQuizFeedback(accuracy, correctCount, totalCount, countedAsCompleted),
    };
  }
