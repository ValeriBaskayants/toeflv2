...
const LIST_SELECT = {
  ...
  level: true, topic: true, tags: true,
  wordCount: true, estimatedMinutes: true, coverImageUrl: true, createdAt: true,
...
export class ReadingsService {
  ...
  async findMany(params: {
    ...
    const userHistory = await this.prisma.userMistake.findMany({
      ...
      select: { targetId: true, correctCount: true, wrongCount: true, status: true },
    });
  ...
  async submitAnswers(
    ...
    const [material, user, existingHistory] = await Promise.all([
      ...
      this.prisma.user.findUnique({ where: { id: userId }, select: { streak: true } }),
    ...
    const results = dto.answers.map((a) => {
      ...
      const isCorrect = question.options[a.selectedOptionIdx]?.isCorrect === true;
      return { questionIdx: a.questionIdx, isCorrect, correctIdx, explanation: question.explanation };
    });
    ...
    const prevBest = existingHistory
      ? Math.round(
        (existingHistory.correctCount /
          (existingHistory.correctCount + existingHistory.wrongCount)) *
        100,
      )
      : 0;
    ...
    const xpEarned = computeXP({ base: baseXP, streak: user?.streak ?? 0, accuracy });

    if (countedAsCompleted) {
      await this.progress.recordSkillCompletion({
        userId, skill: 'reading', accuracy, xpEarned, timezone,
      });
    } else {
    ...
    if (material.grammarTopics.length > 0) {
      await this.topicMastery.recordPractice({
        userId,
        topicSlugs: material.grammarTopics,
        kind: 'reading',
      });
    }
    ...
    await this.prisma.userMistake.upsert({
      ...
      update: {
        ...(isFirstAttempt || accuracy > prevBest
          ? { correctCount, wrongCount: results.length - correctCount }
          : {}),
        status: isMastered ? 'MASTERED' : 'LEARNING',
        updatedAt: new Date(),
      },
    ...
    return {
      results, accuracy, xpEarned, countedAsCompleted,
      bestAccuracy, attemptNumber,
      feedback: buildReadingFeedback(accuracy, isFirstAttempt, correctCount, results.length),
    };
  }
  ...
  async getUserHistory(userId: string) {
    ...
    return history.map((h) => {
      ...
      return { materialId: h.targetId, topic: h.topic, level: h.level, bestAccuracy, status: h.status };
    });
  }
...
function buildReadingFeedback(
  ...
  if (accuracy === 100)
    return `Perfect! All ${total} questions correct${isFirstAttempt ? ' on your first attempt' : ''}.`;
  if (accuracy >= 80)
    return `${correctCount}/${total} correct (${accuracy}%). ${wrong} question${wrong > 1 ? 's' : ''} to review below.`;
  if (accuracy >= 60)
    return `${correctCount}/${total} correct (${accuracy}%). Re-read the tricky sections and try again.`;
  if (accuracy >= 40)
    return `${correctCount}/${total} correct (${accuracy}%). Focus on comprehension — read slowly and check the passage for each question.`;
  return `${correctCount}/${total} correct (${accuracy}%). Try reading the passage again carefully before answering.`;
}