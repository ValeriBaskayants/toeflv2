...
import { MultipleChoiceApi } from '@/api/services/multiple-choice';
...
export const submitQuiz = createAsyncThunk<
  SubmitMCSessionResponse,
  { answers: Record<string, number>; timezone?: string },
  { rejectValue: string }
>('quiz/submit', async ({ answers, timezone }, { rejectWithValue }) => {
  try {
    const payload = {
    ...
    const { data } = await MultipleChoiceApi.submit(payload, timezone);
    return data;
  } catch (err: unknown) {
...
export const quizSlice = createSlice({
  ...
  reducers: {
    ...
    goToQuestion: (state, action: PayloadAction<number>) => {
      ...
      if (idx >= 0 && idx < state.questions.length) state.currentIdx = idx;
    },
    nextQuestion: (state) => {
      if (state.currentIdx < state.questions.length - 1) state.currentIdx += 1;
    },
    prevQuestion: (state) => {
      if (state.currentIdx > 0) state.currentIdx -= 1;
    },
    exitQuiz: (state) => {
      state.phase        = 'setup';
      state.questions    = [];
...
export const {
  updateSetup, selectAnswer, goToQuestion,
  nextQuestion, prevQuestion, exitQuiz, resetQuiz,
} = quizSlice.actions;